...
els.btnVincularLive.addEventListener("click", () => {
  ...
  const avisoValidacao = validarUrlLive(url);
  if (avisoValidacao) {
    mostrarAvisoLive(avisoValidacao);
    return;
  }
  iniciarLive(url);
});
...
// Confere o formato da URL ANTES de gastar uma tentativa de conexão.
// A maioria dos erros de "live" não é CORS — é o usuário colando o link da
// página do canal (kick.com/fulano) em vez do link direto da playlist HLS.
// Essas plataformas não expõem esse link publicamente, então nem vale tentar.
function validarUrlLive(url) {
  let parsed;
  try {
    parsed = new URL(url);
  } catch {
    return 'Isso não parece uma URL válida. Cola o link completo, começando com "https://".';
  }

  if (/\.m3u8(\?|#|$)/i.test(parsed.pathname + parsed.search)) return null; // formato ok, deixa tentar

  const paginasConhecidas = [
    { re: /(^|\.)kick\.com$/i, nome: "Kick" },
    { re: /(^|\.)twitch\.tv$/i, nome: "Twitch" },
    { re: /(^|\.)(youtube\.com|youtu\.be)$/i, nome: "YouTube" },
  ];
  const achou = paginasConhecidas.find((p) => p.re.test(parsed.hostname));
  if (achou) {
    return `Essa é a página do canal/vídeo na ${achou.nome}, não o link direto do stream. O AI Cutter precisa da URL do arquivo .m3u8 (a playlist HLS) — a ${achou.nome} não expõe esse link na página normal, então não dá pra vincular direto por aqui.`;
  }

  return 'Esse link não parece uma URL .m3u8 direta. Confirma se termina em ".m3u8" (ex: https://exemplo.com/live/index.m3u8).';
}
...
async function iniciarLive(url) {
  mostrarAvisoLive("");
...
function dispararCorteLive(live, tag) {
  const emoji = tag === "viral" ? "🚀" : tag === "cena_agitada" ? "🎬" : "🔥";
  ...
  const card = document.createElement("div");
  card.className = "clip-card";
  const corBadge = tag === "viral" ? "var(--viral)" : tag === "cena_agitada" ? "var(--cena)" : "var(--audio)";
