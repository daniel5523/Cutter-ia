...
async function iniciarLive(urlBruta) {
  ...
  try {
    ...
    console.error(err);
    let msg = `Não consegui abrir essa live: ${err.message || err}.`;
    if (plataformaDetectada) {
      const nomePlataforma = plataformaDetectada === "kick" ? "Kick" : "Twitch";
      const paginaCanal = plataformaDetectada === "kick" ? `kick.com/${canalDetectado}` : `twitch.tv/${canalDetectado}`;
      msg +=
        ` O link .m3u8 foi encontrado certinho, mas o servidor de vídeo da ${nomePlataforma} não está` +
        ` liberando acesso de fora do próprio site (CORS) pra esse canal agora — isso varia de canal pra` +
        ` canal e de hora pra hora, e não é algo que dá pra contornar no navegador.` +
        ` Alternativa que sempre funciona: abre ${paginaCanal} num navegador, aperta F12 → aba "Rede"/"Network",` +
        ` digita "m3u8" no filtro, dá play no vídeo, clica no arquivo .m3u8 que aparecer na lista, copia a URL` +
        ` completa (com o token no final) e cola ela aqui — link direto sempre passa.`;
    } else {
      msg += " Confirma se é uma URL .m3u8 direta e se a origem permite acesso de outros domínios (CORS).";
    }
    mostrarAvisoLive(msg);
    els.btnVincularLive.disabled = false;
    els.btnVincularLive.textContent = "Vincular live";
    return;
  }
